...
const PartySearch = ({state, dispatch, reset, useWatch, control}) => {
  ...
  const RenderData = (props) => {
    ...
    <>
      {props.data.filter((x)=>{
        if(x.name.toLowerCase().includes(searchTerm.toLowerCase())|| x.code.toLowerCase().includes(searchTerm.toLowerCase())|| x.types?.toLowerCase().includes(searchTerm.toLowerCase())){
          return x 
        }
        if(searchTerm==""){ 
          return x 
      ...
      <tr key={i} className={`${x.check?"table-select-list-selected":"table-select-list"}`}
        ...
        <td className='pt-1 text-center'>
          {x.types?.split(", ").map((y, i2)=>{return<Tag key={i2} color="purple" className='mb-1'>{y}</Tag>})}
        </td>
